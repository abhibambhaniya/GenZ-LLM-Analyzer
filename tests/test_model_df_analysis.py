...
def test_dense_LLM_prefill():
    ...
    if os.path.exists('/tmp/current_llama_7b_prefill_on_TPU.csv'):
        os.remove('/tmp/current_llama_7b_prefill_on_TPU.csv')

    # Load the golden result
    golden_df = pd.read_csv('./golden/llama_7b_prefill_on_TPU.csv')
    ...
    reloaded_current_df = pd.read_csv('/tmp/current_llama_7b_prefill_on_TPU.csv')
...
def test_dense_LLM_decode():
    # Delete the current CSV file if it exists
    if os.path.exists('/tmp/current_llama_7b_decode_on_TPU.csv'):
        os.remove('/tmp/current_llama_7b_decode_on_TPU.csv')

    # Load the golden result
    golden_df = pd.read_csv('./golden/llama_7b_decode_on_TPU.csv')
    ...
    # Reload the saved current result
    reloaded_current_df = pd.read_csv('/tmp/current_llama_7b_decode_on_TPU.csv')
